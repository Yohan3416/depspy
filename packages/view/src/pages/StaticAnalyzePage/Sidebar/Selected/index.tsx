...
export const Selected = () => {
  ...
  return (
    <div className="bg-bg-layout min-w-90">
      <Card
        variant="borderless"
        className="w-full mb-4 bg-bgLayout"
        title={
          <Flex justify="space-between" align="center">
            <Title level={4} className="color-primaryBase">
              {selectNodeInfo.relativeId}
            </Title>
            <Flex gap="small">
              {selectNodeInfo.isGitChange && (
                <Tag color="yellow">{t("static.gitChanged")}</Tag>
              )}
              {selectNodeInfo.isImportChange && (
                <Tag color="blue">{t("static.importChanged")}</Tag>
              )}
              {selectNodeInfo.isSideEffectChange && (
                <Tag color="red">{t("static.sideEffectChanged")}</Tag>
              )}
            </Flex>
          </Flex>
        }
      >
        <Flex wrap gap="small">
          ...
          <Collapse ghost>
            <Panel
              header={<Text strong>{t("static.gitChanged")}</Text>}
              key="1"
            >
              {selectNodeInfo.curCode ? (
                <div
                  className="scroll-bar rounded-lg p-2"
          ...
          <Collapse ghost>
            <Panel
              header={
                <Text strong>{t("static.sidebar.select.export.remove")}</Text>
              }
              key="4"
            >
              {selectNodeInfo.removedExports.length ? (
                ...
                <Empty
                  description={t("static.sidebar.select.export.noRemove")}
                />
              )}
            </Panel>
          </Collapse>
          ...
          <Collapse ghost>
            <Panel
              header={
                <Text strong>{t("static.sidebar.select.export.render")}</Text>
              }
              key="5"
            >
              {selectNodeInfo.renderedExports.length ? (
                ...
                <Empty
                  description={t("static.sidebar.select.export.noRender")}
                />
              )}
            </Panel>
          </Collapse>
          ...
          <Collapse ghost>
            <Panel
              header={
                <Text strong>{t("static.sidebar.select.import.changed")}</Text>
              }
              key="2"
            >
              {Object.keys(selectNodeInfo.importEffectedNames).length ? (
                <Flex vertical gap="small">
                  {Object.keys(selectNodeInfo.importEffectedNames).map(
                    (Item) => (
                      <Card
                        key={Item}
                        size="small"
                ...
                <Empty
                  description={t("static.sidebar.select.import.noChanged")}
                />
              )}
            </Panel>
          </Collapse>
          ...
          <Collapse ghost>
            <Panel
              header={
                <Text strong>{t("static.sidebar.select.export.changed")}</Text>
              }
              key="3"
            >
              {Object.keys(selectNodeInfo.exportEffectedNamesToReasons)
                    ...
                    (Item) => (
                      ...
                      >
                        {selectNodeInfo.exportEffectedNamesToReasons[Item]
                          .isNativeCodeChange && (
                          <Tag color="purple-inverse" style={{ margin: "4px" }}>
                            Native Code Changed
                          </Tag>
                        )}
                          ...
                          >
                            <Flex wrap gap="small">
                              {selectNodeInfo.exportEffectedNamesToReasons[
                                Item
                              ].importEffectedNames[key].map((value, index) => (
                                <Tag
                                  key={index}
                                  bordered={false}
                                  color="purple-inverse"
                                ...
                                >
                                  {value ||
                                    t(
                                      "static.sidebar.select.export.sideEffect",
                                    )}
                                </Tag>
                              ))}
                            </Flex>
                ...
                <Empty
                  description={t("static.sidebar.select.export.noChanged")}
                />
              )}
            </Panel>
          </Collapse>
